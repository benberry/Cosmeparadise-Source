...
<form action="logintest.php" method="post" enctype="multipart/form-data">
...
if (($handle = fopen($csvfile, "r")) !== FALSE) {	//OPEN CSV
	...
	if($row>1){
			...
			{	//echo "have record<br>";
				...
				{	
					//echo "have store<br>";
					$first_store_pos = strpos( $string , "Store:");
					//echo "first_store_pos:".$first_store_pos."<br>";
					while($counting < 10)
					{	$startpos = strpos($string, 'itemprop="name">', $stoppos);	
						$stoppos = strpos($string, '<span class="watch">', $startpos);						
						if($first_store_pos > $startpos && $first_store_pos < $stoppos)
						{	
