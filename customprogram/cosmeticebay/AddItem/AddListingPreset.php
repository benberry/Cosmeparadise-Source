...
$xajax->register(XAJAX_FUNCTION, "CurrencySwitch");
...
///////////////Start the XAJAX CATEGORY code lines to prepare the requests.///////////		
	...
	function FVF($ecatid, $price, $CurrencyRate, $TotalCost, $TempShipping) {
		...
		if($ecatid != "")	//////////check if category has selected
		{
			...
			$BreakEvenPrice = round($newcost/(0.98-$new_fee_rate),2);
			...
			$Paypal_Fee = $BreakEvenPrice * 0.02;
			$Refer_Calculation = (($BreakEvenPrice - $Final_Value_Fee - $Paypal_Fee)* $CurrencyRate)- $TempShipping;
			...
			{	$Final_Value_Fee = $price * $new_fee_rate;
				...
				$Paypal_Fee = $price * 0.02;	
				$Refer_Calculation = (($price - $Final_Value_Fee - $Paypal_Fee)* $CurrencyRate)- $TempShipping;
			}			
			
			$objResponse = new xajaxResponse();
			$objResponse->assign("FVF","value", $Final_Value_Fee);		
			$objResponse->assign("PPF","value", $Paypal_Fee);		
			$objResponse->assign("BreakEvenPrice","value", $BreakEvenPrice);		
			$objResponse->assign("RECAL","value", $Refer_Calculation);		
	...
	function CurrencySwitch($currency, $update_block) {
		
	$objResponse = new xajaxResponse();
	if($currency == "USD")
	{$html='<tr>
<td>Domestic Shipping</td>
	<td><select name="ShippingService"><option value="Delivery">Delivery(Local Delivery)</option><option value="eBayNowImmediateDelivery">eBayNowImmediateDelivery(eBay Now Immediate Delivery)</option><option value="EconomyShippingFromOutsideUS" SELECTED>EconomyShippingFromOutsideUS(Economy Shipping from outside US)</option><option value="ePacketChina">ePacketChina(ePacket delivery from China)</option><option value="ePacketHongKong">ePacketHongKong(ePacket delivery from Hong Kong)</option><option value="ePacketSingapore">ePacketSingapore(ePacket delivery from Singapore)</option><option value="ePacketThailand">ePacketThailand(ePacket delivery from Thailand)</option><option value="ExpeditedShippingFromOutsideUS">ExpeditedShippingFromOutsideUS(Expedited Shipping from outside US)</option><option value="ExpressInternationalShipping">ExpressInternationalShipping(Express International Shipping)</option><option value="ExpressInternationalShipping_2">ExpressInternationalShipping_2(Express International Shipping)</option><option value="ExpressShippingFromOutsideUS">ExpressShippingFromOutsideUS(Express Shipping from outside US)</option><option value="FedEx2Day">FedEx2Day(FedEx 2Day)</option><option value="FedExExpressSaver">FedExExpressSaver(FedEx Express Saver)</option><option value="FedExGroundDisabled">FedExGroundDisabled(FedEx Ground)</option><option value="FedExHomeDelivery">FedExHomeDelivery(FedEx Ground or FedEx Home Delivery)</option><option value="FedExHomeDeliveryDisabled">FedExHomeDeliveryDisabled(FedEx Home Delivery)</option><option value="FedExPriorityOvernight">FedExPriorityOvernight(FedEx Priority Overnight)</option><option value="FedExSmartPost">FedExSmartPost(FedEx SmartPost)</option><option value="FedExStandardOvernight">FedExStandardOvernight(FedEx Standard Overnight)</option><option value="Freight">Freight(Freight)</option><option value="FreightShipping">FreightShipping(Freight Shipping)</option><option value="LocalDelivery">LocalDelivery(Local Delivery/Pickup)</option><option value="Other">Other(Economy Shipping)</option><option value="Pickup">Pickup(Local Pickup)</option><option value="PromotionalShippingMethod">PromotionalShippingMethod(Promotional Shipping Service)</option><option value="SameDayShipping">SameDayShipping(Same Day Shipping)</option><option value="ShippingMethodExpress">ShippingMethodExpress(Expedited Shipping)</option><option value="ShippingMethodOvernight">ShippingMethodOvernight(One-day Shipping)</option><option value="ShippingMethodStandard">ShippingMethodStandard(Standard Shipping)</option><option value="StandardShippingFromOutsideUS">StandardShippingFromOutsideUS(Standard Shipping from outside US)</option><option value="UPS2DayAirAM">UPS2DayAirAM(UPS Next Day Air AM)</option><option value="UPS2ndDay">UPS2ndDay(UPS 2nd Day Air)</option><option value="UPS3rdDay">UPS3rdDay(UPS 3 Day Select)</option><option value="UPSGround">UPSGround(UPS Ground)</option><option value="UPSNextDay">UPSNextDay(UPS Next Day Air Saver)</option><option value="UPSNextDayAir">UPSNextDayAir(UPS Next Day Air)</option><option value="USPSExpressFlatRateEnvelope">USPSExpressFlatRateEnvelope(USPS Express Mail Flat Rate Envelope)</option><option value="USPSExpressMail">USPSExpressMail(USPS Express Mail)</option><option value="USPSExpressMailFlatRateBox">USPSExpressMailFlatRateBox(USPS Express Mail Flat Rate Box)</option><option value="USPSExpressMailLegalFlatRateEnvelope">USPSExpressMailLegalFlatRateEnvelope(USPS Express Mail Legal Flat Rate Envelope)</option><option value="USPSExpressMailPaddedFlatRateEnvelope">USPSExpressMailPaddedFlatRateEnvelope(USPS Express Mail Padded Flat Rate Envelope)</option><option value="USPSFirstClass">USPSFirstClass(USPS First Class Package)</option><option value="USPSFirstClassLargeEnvelop">USPSFirstClassLargeEnvelop(USPS First Class Large Envelop)</option><option value="USPSFirstClassLetter">USPSFirstClassLetter(USPS First Class Letter )</option><option value="USPSFirstClassParcel">USPSFirstClassParcel(USPS First Class Parcel)</option><option value="USPSGround">USPSGround(US Postal Service Ground)</option><option value="USPSMedia">USPSMedia(USPS Media Mail)</option><option value="USPSParcel">USPSParcel(USPS Parcel Post)</option><option value="USPSPriority">USPSPriority(USPS Priority Mail)</option><option value="USPSPriorityFlatRateBox">USPSPriorityFlatRateBox(USPS Priority Mail Medium Flat Rate Box)</option><option value="USPSPriorityFlatRateEnvelope">USPSPriorityFlatRateEnvelope(USPS Priority Mail Flat Rate Envelope)</option><option value="USPSPriorityMailLargeFlatRateBox">USPSPriorityMailLargeFlatRateBox(USPS Priority Mail Large Flat Rate Box)</option><option value="USPSPriorityMailLegalFlatRateEnvelope">USPSPriorityMailLegalFlatRateEnvelope(USPS Priority Mail Legal Flat Rate Envelope)</option><option value="USPSPriorityMailPaddedFlatRateEnvelope">USPSPriorityMailPaddedFlatRateEnvelope(USPS Priority Mail Padded Flat Rate Envelope)</option><option value="USPSPriorityMailRegionalBoxA">USPSPriorityMailRegionalBoxA(USPS Priority Mail Regional Box A)</option><option value="USPSPriorityMailRegionalBoxB">USPSPriorityMailRegionalBoxB(USPS Priority Mail Regional Box B)</option><option value="USPSPriorityMailRegionalBoxC">USPSPriorityMailRegionalBoxC(USPS Priority Mail Regional Box C)</option><option value="USPSPriorityMailSmallFlatRateBox">USPSPriorityMailSmallFlatRateBox(USPS Priority Mail Small Flat Rate Box)</option><option value="USPSStandardPost">USPSStandardPost(USPS Standard Post)</option><option value="US_FedExIntlEconomy">US_FedExIntlEconomy(FedEx International Economy)</option></select></td>
	<td>Cost:</td>
...
<tr>
	...
	<td><select name="InternationalShippingService"><option value="ExpeditedDeliveryToRussia">ExpeditedDeliveryToRussia(Экспресс доставка в Россию)</option><option value="ExpeditedInternational">ExpeditedInternational(Expedited Int\'l Shipping)</option><option value="FedExGroundInternationalToCanada">FedExGroundInternationalToCanada(FedEx Ground International for Canada)</option><option value="FedExInternationalEconomy">FedExInternationalEconomy(FedEx International Economy)</option><option value="FedExInternationalPriority">FedExInternationalPriority(FedEx International Priority)</option><option value="FreightShippingInternational">FreightShippingInternational(Freight Shipping)</option><option value="InternationalPriorityShipping">InternationalPriorityShipping(International Priority Shipping)</option><option value="OtherInternational">OtherInternational(Economy Int\'l Shipping)</option><option value="StandardInternational" SELECTED>StandardInternational(Standard Int\'l Shipping)</option><option value="UPSStandardToCanada">UPSStandardToCanada(UPS Standard To Canada)</option><option value="UPSWorldWideExpedited">UPSWorldWideExpedited(UPS Worldwide Expedited)</option><option value="UPSWorldWideExpress">UPSWorldWideExpress(UPS Worldwide Express)</option><option value="UPSWorldWideExpressBox10kg">UPSWorldWideExpressBox10kg(UPS Worldwide Express Box 10Kg)</option><option value="UPSWorldWideExpressBox25kg">UPSWorldWideExpressBox25kg(UPS Worldwide Express Box 25Kg)</option><option value="UPSWorldWideExpressPlus">UPSWorldWideExpressPlus(UPS Worldwide Express Plus)</option><option value="UPSWorldWideExpressPlusBox10kg">UPSWorldWideExpressPlusBox10kg(UPS Worldwide Express Plus Box 10Kg)</option><option value="UPSWorldWideExpressPlusBox25kg">UPSWorldWideExpressPlusBox25kg(UPS Worldwide Express Plus Box 25Kg)</option><option value="UPSWorldwideSaver">UPSWorldwideSaver(UPS Worldwide Saver)</option><option value="USPSAirmailLetter">USPSAirmailLetter(USPS Airmail Letter Post)</option><option value="USPSAirmailParcel">USPSAirmailParcel(USPS Airmail Parcel Post)</option><option value="USPSEconomyLetter">USPSEconomyLetter(USPS Economy Letter Post)</option><option value="USPSEconomyParcel">USPSEconomyParcel(USPS Economy Parcel Post)</option><option value="USPSExpressMailInternational">USPSExpressMailInternational(USPS Express Mail International)</option><option value="USPSExpressMailInternationalFlatRateBox">USPSExpressMailInternationalFlatRateBox(USPS Express Mail International Flat Rate Box)</option><option value="USPSExpressMailInternationalFlatRateEnvelope">USPSExpressMailInternationalFlatRateEnvelope(USPS Express Mail International Flat Rate Envelope)</option><option value="USPSExpressMailInternationalLegalFlatRateEnvelope">USPSExpressMailInternationalLegalFlatRateEnvelope(USPS Express Mail International Legal Flat Rate Envelope)</option><option value="USPSExpressMailInternationalPaddedFlatRateEnvelope">USPSExpressMailInternationalPaddedFlatRateEnvelope(USPS Express Mail International Padded Flat Rate Envelope)</option><option value="USPSFirstClassMailInternational">USPSFirstClassMailInternational(USPS First Class Mail International)</option><option value="USPSFirstClassMailInternationalLargeEnvelope">USPSFirstClassMailInternationalLargeEnvelope(USPS First Class Mail International Large Envelope)</option><option value="USPSFirstClassMailInternationalLetter">USPSFirstClassMailInternationalLetter(USPS First Class Mail International Letter)</option><option value="USPSFirstClassMailInternationalParcel">USPSFirstClassMailInternationalParcel(USPS First Class Mail International Parcel)</option><option value="USPSGlobalExpress">USPSGlobalExpress(USPS Global Express Mail)</option><option value="USPSGlobalExpressGuaranteed">USPSGlobalExpressGuaranteed(USPS Global Express Guaranteed)</option><option value="USPSGlobalPriority">USPSGlobalPriority(USPS Global Priority Mail)</option><option value="USPSGlobalPriorityLargeEnvelope">USPSGlobalPriorityLargeEnvelope(USPS Global Priority Mail Flat Rate Large Envelope)</option><option value="USPSGlobalPrioritySmallEnvelope">USPSGlobalPrioritySmallEnvelope(USPS Global Priority Mail Flat Rate Small Envelope)</option><option value="USPSPriorityMailInternational">USPSPriorityMailInternational(USPS Priority Mail International)</option><option value="USPSPriorityMailInternationalFlatRateBox">USPSPriorityMailInternationalFlatRateBox(USPS Priority Mail International Medium Flat Rate Box)</option><option value="USPSPriorityMailInternationalFlatRateEnvelope">USPSPriorityMailInternationalFlatRateEnvelope(USPS Priority Mail International Flat Rate Envelope)</option><option value="USPSPriorityMailInternationalLargeFlatRateBox">USPSPriorityMailInternationalLargeFlatRateBox(USPS Priority Mail International Large Flat Rate Box)</option><option value="USPSPriorityMailInternationalLegalFlatRateEnvelope">USPSPriorityMailInternationalLegalFlatRateEnvelope(USPS Priority Mail International Legal Flat Rate Envelope)</option><option value="USPSPriorityMailInternationalPaddedFlatRateEnvelope">USPSPriorityMailInternationalPaddedFlatRateEnvelope(USPS Priority Mail International Padded Flat Rate Envelope)</option><option value="USPSPriorityMailInternationalSmallFlatRateBox">USPSPriorityMailInternationalSmallFlatRateBox(USPS Priority Mail International Small Flat Rate Box)</option></select></td>
	<td>Cost:</td>
...
<tr>
	...
	$objResponse->assign($update_block,"innerHTML", $html);		
	return $objResponse;//->getXML();
...
<?php
	...
	{	
		...
		$sql = 'SELECT * FROM `item_able_to_ebay` WHERE id IN ('.$product_ids.') ORDER BY mpn ASC;';		
...
<td><select onChange="xajax_CurrencySwitch(this[this.selectedIndex].value, 'ship_with_site')" class="selectinput" name="Currency"><option value="AUD" SELECTED>Australia Dollar</option><option value="USD">United States Dollar</option><!-- <option value="GBP">United Kingdom Pound</option><option value="EUR">Euro Member Countries</option>--></td>
</tr><tr>
...
<td>ListingDuration</td><td><select class="selectinput" name="ListingDuration"><option value="GTC" SELECTED>GTC</option><option value="Days_3">3 Days</option><option value="Days_5">5 Days</option><option value="Days_7">7 Days</option><option value="Days_10">10 Days</option><option value="Days_14">14 Days</option><option value="Days_21">21 Days</option><option value="Days_30">30 Days</option></select></td>
...
<td>&nbsp;</td><td>&nbsp;</td><td>Paypal Fee(2%)</td><td><input type="text" name="PPF" id="PPF" /></td><td>Refer Cal.</td><td colspan="3"><input type="text" name="RECAL" id="RECAL" /></td>
</tr>
</table>
...
<tr><td>Returns Accepted</td>
	...
	<td><select class="selectinput" name="RefundOption"><option value="MoneyBack">MoneyBack</option><option value="MoneyBackOrExchange">MoneyBackOrExchange(For US)</option><option value="Exchange" SELECTED>Exchange(For AU)</option></select></td>
	<td><select class="selectinput" name="ReturnsWithinOption" id="ReturnsWithinOption"><option value=""></option><option value="Days_3">3 Days</option><option value="Days_7">7 Days</option><option value="Days_14" SELECTED>14 Days</option><option value="Days_30">30 Days</option><option value="Days_60">60 Days</option><option value="Months_1">1 Month</option></select></td>
</tr>
