...
<?php
	...
	$sql = 'SELECT SKU FROM `cosme_ebay_listing`';
...
if($_POST["Title"] != "" || $_POST["SKU"] != "" || $_POST["ItemID"] != "" || ISSET($_POST["CategoryName"])) {
	...
	try {  
		...
		$sql = 'SELECT * FROM `cosme_ebay_listing` CBL'.$Title.$ItemID.$SKU.$CategoryName.$Site.' GROUP BY ItemID'.$SortBy.$page_item.';';
		//echo $sql;
		$BulkUpdateItems = "";
		$result = mysqli_query($con, $sql);	
		while($row = mysqli_fetch_array($result))
	...
	{	require('../get-common/keys.php');
		...
		{			
			...
			if(count($longMsg) > 0)
			 ...
			 $sql = 'UPDATE cosme_ebay_listing SET Title = "'.$Item_Title.'", CurrentPrice = '.(double)$Price.', QuantityAvailable = '.$Qty.' WHERE ItemID='.$ItemID.';';	
