...
<html>
<body>
<h2>Grab ebay UK price programme by name</h2>
<b>Please keep the format: sku, name(SKIP first row of header)</b>
<form action="ebaygrabprice_uk.php" method="post" enctype="multipart/form-data">
...
if (($handle = fopen($csvfile, "r")) !== FALSE) {	//OPEN CSV
	...
	if($row>1){
			...
			$newlink = 0;
			$product_data = array('_nkw' => $Product_Name, '_sop' => '15', 'LH_ItemCondition' => '3',  'LH_BIN' => '1' );
			...
			{	//echo "have record<br>";
				...
				if(strpos( $string , "Shop:") !== false)	/////if have Shop
				{	
					//echo "have Shop<br>";
					$Shop_pos = strpos( $string , "Shop:");
					//echo "Shop_pos:".$Shop_pos."<br>";
					while($counting < 10 && $counting < (int)$record_count)
					{	$startpos = strpos($string, 'itemprop="name">', $stoppos);	
						$stoppos = strpos($string, '</table>', $startpos);						
						if($Shop_pos > $startpos && $Shop_pos < $stoppos)
						{	
							...
							//////////filter name out////////////
								...
								{	$Shop_pos = strpos( $string , "Shop:", $stoppos);
							...
							///////////////find Shop name/////////////
							$Shop_start = strpos($string, '<span class="v">', $name_stop)+strlen('<span class="v">');
							$Shop_stop = strpos($string, '</a>', $Shop_start);
							$Shop_name = strip_tags(substr($string, $Shop_start, $Shop_stop-$Shop_start));//remove tag
							//echo "Shop_name:".$Shop_name."<br>";
							//////////////find price///////////////////
							$price_start = strpos($string, '<div  class="g-b" itemprop="price">', $Shop_stop)+strlen('<div  class="g-b" itemprop="price">');
							...
							if(strpos( $string , '<span class="fee">', $startpos) !== false && strpos( $string , '<span class="fee">', $startpos) > $startpos && strpos( $string , '<span class="fee">', $startpos) < $stoppos)	//////if have shipping cost
							{
								...
								$shipping = str_replace(" postage", "", $shipping);
								$shipping = str_replace("+ £", "", $shipping);
							}
							else if(strpos( $string , "<span class='tfsp'>", $startpos) !== false && strpos( $string , "<span class='tfsp'>", $startpos) > $startpos && strpos( $string , "<span class='tfsp'>", $startpos) < $stoppos)	//////if show free shipping
								$shipping = "Free shipping";
							else
								$shipping = "Shipping not specified";
							//echo "shipping:".$shipping."<br>";	
							
							if($Shop_name != "Cosme Paradise") 
							...
							$Enclosure.$Shop_name.$Enclosure.$Separator.	
...
/////////////open file and write into it///////////////
	$filename = "./grabprice/ebaygrabprice_uk.csv";
	$ourFileHandle = fopen($filename, 'w') or die("can't open file");
	fputs($ourFileHandle, $line);
	//close file after write the content into it
	fclose($ourFileHandle);
	
	echo "<br><br>success. check file link <a href=\"./grabprice/ebaygrabprice_uk.csv\">Here</a><br>";
...
$userid    = 'support@analystsupporter.com';
$pass = 'U010nduty';

// initial login page which redirects to correct sign in page, sets some cookies
$URL = 'https://signin.ebay.co.uk/ws/eBayISAPI.dll?SignIn&ru=http%3A%2F%2Fwww.ebay.co.uk%2F';
