...
<html>
<body>
<h2>Grab Kogan price programme by link</h2>
<b>Please keep the format: sku/id, Name, URL(SKIP first row of header)</b>
<form action="kogangrabprice.php" method="post" enctype="multipart/form-data">
...
/////////////open file and write into it///////////////
	/*$filename = "./kogangrabprice.csv";
	$ourFileHandle = fopen($filename, 'w') or die("can't open file");
	fputs($ourFileHandle, $line);
	//close file after write the content into it
	fclose($ourFileHandle);
	
	echo "<br><br>success. check file link <a href=\"./kogangrabprice.csv\">Here</a><br>";
...
{
	$cookieFile = '/tmp/kogancookie.txt';
	...
	if(substr($URL, -1) == "/")
		$URL_add_to_cart = $URL."cart/";
	else
		$URL_add_to_cart = $URL."/cart/";
	echo $URL_add_to_cart."<br>";
	$URL_cart = 'https://www.kogan.com/au/cart/';
	...
	if (!preg_match('/<form method="POST" id="form-add-to-cart" action=.*?<\/form>/is', $page, $form)) {
		die('Failed to find product form!');
	}
	...
	/*$ch  = curl_init();
	curl_setopt($ch, CURLOPT_URL, $URL);
	curl_setopt($ch, CURLOPT_REFERER, $URL_add_to_cart);
	curl_setopt($ch, CURLOPT_COOKIESESSION, true);
	curl_setopt($ch, CURLOPT_COOKIEJAR, $cookieFile);
	curl_setopt($ch, CURLOPT_COOKIEFILE, $cookieFile);
	curl_setopt($ch, CURLOPT_USERAGENT, 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/31.0.1650.63 Safari/537.36');
