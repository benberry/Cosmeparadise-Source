<?php
	...
	$sql = "";
	if($NotificationEventName == "ItemRevised" || $NotificationEventName == "ItemSold")
	{	$sql = "UPDATE cosme_ebay_listing SET Title ='".str_replace("'","''",$Title)."', QuantityAvailable = ".$QuantityAvailable.", CurrentPrice = ".$CurrentPrice.", EndTime ='".$EndTime."' WHERE ItemId = ".$ItemID;
		
	}else if($NotificationEventName == "ItemListed"){
		$sql = "SELECT COUNT(ItemId) FROM cosme_ebay_listing WHERE ItemId=".$ItemID;
		list($ItemId_COUNT) = mysqli_fetch_row(mysqli_query($con, $sql));
		if($ItemId_COUNT > 0) ////////ItemID exist, it's relist
			$sql = "UPDATE cosme_ebay_listing SET Title ='".str_replace("'","''",$Title)."', QuantityAvailable = ".$QuantityAvailable.", CurrentPrice = ".$CurrentPrice.", EndTime ='".$EndTime."' WHERE ItemId = ".$ItemID;
		else	//////////new listing
			$sql = "INSERT INTO cosme_ebay_listing VALUES ('".str_replace("'","''",$BuyerProtection)."', '".$Currency."', ".$ItemID.", '".str_replace("'","''",$EndTime)."', '".str_replace("'","''",$ViewItemURL)."', '".str_replace("'","''",$ListingDuration)."', '".str_replace("'","''",$ListingType)."', ".$CategoryID.", '".str_replace("'","''",$CategoryName)."', ".$Quantity.", ".$QuantitySold.", ".$QuantityAvailable.", ".$CurrentPrice.", '".str_replace("'","''",$ShipToLocations)."', '".str_replace("'","''",$Site)."', '".str_replace("'","''",$Title)."', '".str_replace("'","''",$SKU)."', ".$HitCount.", ".$ConditionID.", '".str_replace("'","''",$ConditionDisplayName)."', '".str_replace("'","''",$PictureURL)."', ".$ShippingProfileID.", '".str_replace("'","''",$ShippingProfileName)."', ".$ReturnProfileID.", '".str_replace("'","''",$ReturnProfileName)."', ".$PaymentProfileID.", '".str_replace("'","''",$PaymentProfileName)."');";
	
	}else if($NotificationEventName == "ItemClosed"){
		$sql = "DELETE FROM cosme_ebay_listing WHERE ItemId=".$ItemID;
	}else{
		$Update_Date .= "\r\n".$dataPOST;
	}
