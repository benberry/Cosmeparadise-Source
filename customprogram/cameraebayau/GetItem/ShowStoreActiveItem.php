...
if ($ip == "61.93.89.14" ) $accesslist=true;	//company IP Jimmy
...
<?php
	...
	$sql = 'SELECT SKU FROM `camau_ebay_listing`';
	...
	$sql = "SELECT distinct(CategoryName) FROM `camau_ebay_listing` ORDER BY CategoryName;";
	$result = mysqli_query($con, $sql);	
	...
	$sql = "SELECT distinct(Site) FROM `camau_ebay_listing`;"; 					
	$result = mysqli_query($con, $sql);	
	while($row = mysqli_fetch_array($result))
	{	$site_count++;
...
<td colspan="4">Total current Listing:<?php echo $Total_Active_Listing ?></td>
</table>
</form>
...
if($_POST["Title"] != "" || $_POST["SKU"] != "" || $_POST["ItemID"] != "" || ISSET($_POST["CategoryName"])) {
	...
	if($_POST["BulkUpdate"] == "Confirm")
		{	$total_items = count($_POST['ItemIDs']);
			...
			{	// Calculate the percentation
				...
				echo '<script language="javascript">document.getElementById("progress").innerHTML="<div style=\"width:'.$percent.';background-image:url(http://www.cosmeparadise.com/customprogram/cameraebayau/pbar-ani.gif);\">&nbsp;</div>";document.getElementById("information").innerHTML="'.$i.' Item(s) processed.";</script>';		
				// This is for the buffer achieve the minimum size in order to flush data
				echo str_repeat(' ',1024*64);
			...
			// Javascript for updating the progress bar and information
			echo '<script language="javascript">document.getElementById("progress").innerHTML="<div style=\"width:100%;background-image:url(http://www.cosmeparadise.com/customprogram/cameraebayau/pbar-ani.gif);\">&nbsp;</div>";    document.getElementById("information").innerHTML="Process completed";</script>';
			// This is for the buffer achieve the minimum size in order to flush data
			echo str_repeat(' ',1024*64);
			// Send output to browser immediately
			flush();
	...
	try {  
		...
		$sql = 'SELECT * FROM `camau_ebay_listing` CBL'.$Title.$ItemID.$SKU.$CategoryName.$Site.' GROUP BY ItemID'.$SortBy.$page_item.';';
		//echo $sql;
		$BulkUpdateItems = "";
		$result = mysqli_query($con, $sql);	
		while($row = mysqli_fetch_array($result))
	...
	{	require('../get-common/keys.php');
		...
		{			
			...
			if(count($longMsg) > 0)
			 ...
			 $sql = 'UPDATE camau_ebay_listing SET Title = "'.$Item_Title.'", CurrentPrice = '.(double)$Price.', QuantityAvailable = '.$Qty.' WHERE ItemID='.$ItemID.';';	
