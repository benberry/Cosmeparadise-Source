...
$onemonthbefore = date( "Y/m/d", strtotime ("-10 day", strtotime($today) ) ); 
$SBN_url = "http://affiliate.strawberrynet.com/affiliate/cgi/statusResponse.aspx?siteId=cosmeparadise&dateFrom=".$onemonthbefore."&dateTo=".$today."&remark=1&product=1";
...
{ 
	...
	foreach($xml->children() as $Order){
		...
		if( (int)$orderNo > 100000000 && (int)$orderNo < 199999999)
		...
		{				
			...
			//echo "order:".$orderNo." have been cancelled<br>";
		}
		}
	}
}
  ...
  function completeAndShip($orderNo, $trackingNum, $carrierTitle, $productlist, $connection_read, $connection_write){
        ...
        foreach ($order->getAllItems() as $orderItem) {
			...
			echo "item name:".$orderItem->getName()."--";
			if(strpos($orderItem->getName(), "FREE GIFT") === false)
			{	
				$product = Mage::getModel('catalog/product')->load($orderItem->getProductId());
			 
				//////////////////////////get current_product_type///////////////////////////////////
				if ($product->getResource()->getAttribute('current_product_type'))
				{	if($product->getResource()->getAttribute('current_product_type')->getFrontend()->getValue($product) != "SBN")
					{	$allsbn = false;
						continue;
					}//else
					 //	$existsbn = true;
		...
		else if($allsbn == false && $existsbn == true)		
			$comment = "Order has been shipped from different warehouse and splited into two parcels, sorry for any inconvenience caused and thank you for your understanding.";	
		
		if($existsbn == true)
		{
		if($carrierTitle == 'SpeedPost')
			$carrierTitle = 'Registered Airmail';
		else if($carrierTitle == 'USPS')
			$carrierTitle = 'USPS';
		else
			$carrierTitle = 'Registered Post';
			
		/*else if($carrierTitle == 'Australia Post')
			$carrierTitle = 'Registered Post';
		else if($carrierTitle == 'PostNord')
			$carrierTitle = 'Registered Post';
		else if($carrierTitle == 'Royal Mail')
			$carrierTitle = 'Registered Post';*/		
